...
# config.origin values produced by Alpha Factory idea generation
# (alpha_factory_search, backtest_worker.py) -- PySR symbolic regression and
# stable-baselines3 PPO both generate a candidate Strategy from ~4.5 hours
# of real tick history in this dev environment. There is no way to tell
# whether a generated candidate has genuine edge versus noise on that little
# data -- "it ran and produced a row" is the only available check, not
# "it works" -- so a generated candidate is quarantined at every stage gate
# until a human sets config.alpha_factory_reviewed = true via the ordinary
# PATCH /strategies/{id} path (config is already patchable, admin/quant-
# gated at the router). This is the only thing standing between an
# unverifiable-on-this-data candidate and the same BACKTEST->PAPER->LIVE
# pipeline every human-authored strategy uses.
ALPHA_FACTORY_ORIGINS = frozenset({"alpha_factory_pysr", "alpha_factory_rl"})
...
def _check_gate(strategy: Strategy, to_stage: str) -> tuple[bool, str]:
    ...
    """
    if (
        (strategy.config or {}).get("origin") in ALPHA_FACTORY_ORIGINS
        and not (strategy.config or {}).get("alpha_factory_reviewed")
    ):
        return (
            False,
            f"Strategy was auto-generated by Alpha Factory ({strategy.config.get('origin')}) "
            "and is quarantined pending human review -- set config.alpha_factory_reviewed=true "
            "(PATCH /strategies/{id}) before it can advance",
        )
